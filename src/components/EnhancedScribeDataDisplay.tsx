...
import { ChevronDown, ChevronUp, CheckCircle, AlertCircle, XCircle, RefreshCw, Mic, Clock, TrendingUp, Activity } from 'lucide-react';
...
export const EnhancedScribeDataDisplay = forwardRef<EnhancedScribeDataDisplayRef, EnhancedScribeDataDisplayProps>(({
    ...
    try {
      if (patientId) {
        const {
          data,
          error
        } = await supabase.from('audio_transcripts').select('id, transcript_text, processed, created_at, assessment_id, patient_assessments!inner(patient_id)').eq('patient_assessments.patient_id', patientId).order('created_at', {
          ascending: false
        }).limit(100);
        if (error) {
        ...
        const {
          data,
          error
        } = await supabase.from('audio_transcripts').select('*').eq('assessment_id', assessmentId!).order('created_at', {
          ascending: false
        });
        if (error) {
    ...
    try {
      if (patientId) {
        const {
          data,
          error
        } = await supabase.from('form_field_values').select('id, field_id, section_id, field_label, value, data_source, ai_source_text, created_at, assessment_id, patient_assessments!inner(patient_id)').eq('data_source', 'ai-filled').eq('patient_assessments.patient_id', patientId).order('created_at', {
          ascending: false
        }).limit(200);
        if (error) {
          ...
          const validations = validateExtractedFields(fields.map((field: any) => ({
            fieldId: field.field_id,
            value: field.value,
            confidenceScore: 0.8
          })));
          const validatedDisplayData: FieldValidationDisplay[] = fields.filter((field: any) => field.field_id && field.value).map((field: any) => {
        ...
        const {
          data,
          error
        } = await supabase.from('form_field_values').select('*').eq('assessment_id', assessmentId!).eq('data_source', 'ai-filled').order('created_at', {
          ascending: false
        });
        if (error) {
          ...
          const fields = data || [];
          setAiFilledFields(fields);
          const validations = validateExtractedFields(fields.map(field => ({
            fieldId: field.field_id,
            value: field.value,
            confidenceScore: 0.8
          })));
          const validatedDisplayData: FieldValidationDisplay[] = fields.filter(field => field.field_id && field.value).map(field => {
            const validation = validations.find(v => v.fieldId === field.field_id);
            return {
              fieldData: field as FormFieldValue,
              validation: validation || {
  ...
  useEffect(() => {
    ...
    const transcriptsSubscription = supabase.channel('audio_transcripts_changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
    ...
    const fieldsSubscription = supabase.channel('form_field_values_changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  return <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-xl font-bold">AI-Extracted Data</CardTitle>
            </div>
          ...
          {/* Validation Summary */}
          {validationSummary.totalFields > 0 && <div className="flex items-center gap-4 pt-2">
              <Badge variant="outline" className="flex items-center gap-1">
                <CheckCircle className="h-3 w-3 text-green-600" />
                {validationSummary.validFields} Valid
              </Badge>
              {validationSummary.fieldsWithErrors > 0 && <Badge variant="destructive" className="flex items-center gap-1">
                  <XCircle className="h-3 w-3" />
                  {validationSummary.fieldsWithErrors} Errors
                </Badge>}
              {validationSummary.fieldsWithWarnings > 0 && <Badge variant="secondary" className="flex items-center gap-1">
        ...
        <CardContent>
          {validatedFields.length === 0 ? <div className="text-center py-8 text-muted-foreground">
            ...
            </div> : <Collapsible open={isValidationExpanded} onOpenChange={setIsValidationExpanded}>
              <CollapsibleTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-2 w-full justify-start p-0 h-auto">
                  ...
                  {validationSummary.criticalErrors.length > 0 && <Badge variant="destructive" className="ml-2">
              ...
              <CollapsibleContent className="space-y-2 mt-4">
                <ScrollArea className="h-[400px]">
                  <div className="space-y-3">
                    {validatedFields.map((fieldDisplay, index) => <div key={`${fieldDisplay.fieldData.id}-${index}`} className={`p-3 rounded-lg border ${!fieldDisplay.validation.validation.isValid ? 'border-red-200 bg-red-50' : fieldDisplay.validation.validation.warnings.length > 0 ? 'border-yellow-200 bg-yellow-50' : 'border-green-200 bg-green-50'}`}>
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="mt-1">
                              ...
                              {fieldDisplay.validation.originalValue !== fieldDisplay.validation.validation.normalizedValue && <span className="text-xs text-muted-foreground ml-2">
                                  (normalized from: {fieldDisplay.validation.originalValue})
                                </span>}
                            </div>
                            
                            {/* Validation Messages */}
                            {fieldDisplay.validation.validation.errors.length > 0 && <div className="mt-2">
                                {fieldDisplay.validation.validation.errors.map((error, idx) => <div key={idx} className="flex items-center gap-1 text-xs text-red-600">
                                    <XCircle className="h-3 w-3" />
                                    {error}
                                  </div>)}
                              </div>}
                            
                            {fieldDisplay.validation.validation.warnings.length > 0 && <div className="mt-2">
                                {fieldDisplay.validation.validation.warnings.map((warning, idx) => <div key={idx} className="flex items-center gap-1 text-xs text-yellow-600">
                            ...
                            {/* AI Source Text */}
                            {fieldDisplay.fieldData.ai_source_text && <div className="mt-2 text-xs text-muted-foreground">
                                <strong>Source:</strong> "{fieldDisplay.fieldData.ai_source_text}"
                              </div>}
                          </div>
                          
                          <div className="flex items-center gap-1 ml-3">
                            {fieldDisplay.validation.validation.isValid ? <CheckCircle className="h-4 w-4 text-green-600" /> : <XCircle className="h-4 w-4 text-red-600" />}
      ...
      <Card>
        <Collapsible open={isHistoryExpanded} onOpenChange={setIsHistoryExpanded}>
          ...
          <CollapsibleContent>
            <CardContent>
              {scribeHistory.length === 0 ? <div className="text-center py-6 text-muted-foreground">
                  <Clock className="h-8 w-8 mx-auto mb-2 opacity-50" />
                  <p>No transcription history yet.</p>
                </div> : <ScrollArea className="h-[300px]">
                  <div className="space-y-3">
                    {scribeHistory.map((entry, index) => <div key={entry.id} className="border rounded-lg p-3">
